...
def _setup_logging(run_dir: Path, tag: str) -> logging.Logger:
    ...
    logger = logging.getLogger("roberta_joint")
...
def _eval_st2(model, ext_dev_records, device, batch_size: int = 32) -> float:
    pad_id = 1
    f1s = []
...
def eval_all(model, dev_loaders, ext_dev_records, device) -> dict:
    model.eval()
    metrics = {}
    with torch.no_grad():
        metrics.update(_eval_cls(model, 0, dev_loaders[0], device))
        metrics["span_f1"] = _eval_st2(model, ext_dev_records, device)
...
def train_one(
    ...
    for epoch in range(C.EPOCHS):
        ...
        metrics = eval_all(model, dev_loaders, ext_dev, device)
        logger.info(
    ...
    model.to(device)
    return eval_all(model, dev_loaders, ext_dev, device)
